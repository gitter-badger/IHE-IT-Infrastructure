...
//        }
    ...
    public List<OMElement> getParameters() {
        if(patientId != null) {
            super.list.add(super.addParameter("$XDSSubmissionSetPatientId", patientId));
        }else {
            return null;
        }
        if(status != null) {
            super.list.add(super.addParameter("$XDSSubmissionSetStatus", status));
        }else {
            return null;
        }
        if(submissionTimeFrom != null && submissionTimeTo != null) {
            DateFormat dateFormat = new SimpleDateFormat("yyyyMMddHHmmss");
            super.list.add(super.addParameter("$XDSSubmissionSetSubmissionTimeFrom", dateFormat.format(this.getSubmissionTimeFrom())));
            super.list.add(super.addParameter("$XDSSubmissionSetSubmissionTimeTo", dateFormat.format(this.getSubmissionTimeTo())));
        }
        if(sourceId != null) {
            super.list.add(super.addParameter("$XDSSubmissionSetSourceId", sourceId));
        }
        if(authorPerson != null) {
            super.list.add(super.addParameter("$XDSSubmissionSetAuthorPerson", authorPerson));
        }
        if(this.getContentTypeCode() != null) {
            Code code = this.getContentTypeCode();
            String value = code.getCode() + "^^" + code.getCodingScheme();
            super.list.add(super.addParameter("$XDSSubmissionSetContentType", value));
        }
        return super.list;
    }
