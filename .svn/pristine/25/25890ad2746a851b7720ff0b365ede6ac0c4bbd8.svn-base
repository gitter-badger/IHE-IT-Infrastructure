...
public class Sha1Bean {
	...
	public static Logger logger = Logger.getLogger(Sha1Bean.class);
	 ...
	 * 
	 * @param byteStream
	 *            New value of property byteStream.
	 */
	...
	public String getSha1(byte[] bytes) throws Exception {
		setByteStream(bytes);
		return getSha1String();
	}
	...
	public static void main(String[] argv) throws Exception {
		...
		// Common.logger.info("sha1 of 'Jelani Nelson' is " +
		// sb.getSha1String());

		String fin = Io.stringFromFile(new File(
				"temp5915305201903712751my_document.xml"));
		byte[] inb = fin.getBytes();
		Sha1Bean sb = new Sha1Bean();
		sb.setByteStream(inb);
		logger.info(sb.getSha1String());

		// fin = Io.stringFromFile(new File("/Users/bill/tmp/hash/rep.txt"));
		// inb = fin.getBytes();
		// sb = new Sha1Bean();
		// sb.setByteStream(inb);
		// Common.logger.info("sha1 of 'rep.txt' is " + sb.getSha1String());
	}
}
