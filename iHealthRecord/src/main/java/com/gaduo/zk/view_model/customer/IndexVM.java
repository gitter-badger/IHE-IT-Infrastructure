...
import com.gaduo.webservice.LeafClass.MetadataParser.Entry;
...
import com.gaduo.zk.model.list_item.Item;
...
//				if (!set.isEmpty() && (t[i] == null || !t[i].isAlive())) {
//					DistributedGetFolders gf = new DistributedGetFolders();
//					gf.setId(next);
//					t[i] = new Thread(gf); // 產生Thread物件
//					t[i].start(); // 開始執行Runnable.run();
//					list.add(new Item(next, gf.getLastUpdateTime(), gf
//							.getTitle(), gf.getDescription(), Item.FOLDER));
//				}
//			}
