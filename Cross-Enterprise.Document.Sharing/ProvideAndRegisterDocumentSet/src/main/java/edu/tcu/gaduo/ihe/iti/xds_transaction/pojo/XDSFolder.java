...
public class XDSFolder extends XDSEntry {
	...
	public void build(){
		...
		if (lastUpdateTime != null) {
			OMElement slot = this.addSlot(FolderConstants.LAST_UPDATE_TIME, new String[] { lastUpdateTime });
			root.addChild(slot);
		}
		...
		if (value != null) {
			buildClassification("folderCodeList", value, FolderConstants.CODING_SCHEME, this.getId(), ProvideAndRegistryDocumentSet_B_UUIDs.FOLDER_CODE);
		}
		// ---------------------ExternalIdentifier
		...
		addExternalIdentifier(ProvideAndRegistryDocumentSet_B_UUIDs.FOLDER_PATIENT_IDENTIFICATION_SCHEME, this.getId(), sourcePatientId, name);
		...
		addExternalIdentifier(ProvideAndRegistryDocumentSet_B_UUIDs.FOLDER_UNIQUE_IDENTIFICATION_SCHEME, this.getId(), uniqueId, name);
	...
	public void setFolder(OMElement folder) {
		this.folder = folder;
	}
