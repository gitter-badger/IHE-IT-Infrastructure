...
//	private NonBlockCallBack callback;
	...
	public OMElement QueryGenerator(OMElement source) {
		...
		common.saveLog(filename, common.SOURCE + "_" + getQueryType(QueryUUID), source);

		QueryGenerator q = new QueryGenerator();
		OMElement request = q.execution(source);
		common.saveLog(filename, common.ITI_18_REQUEST + "_" + getQueryType(QueryUUID), request);
...
//			SOAPEnvelope envelope = (callback != null) ? callback.getEnvelope() : null;
...
//			res.parser(callback.getEnvelope());
			...
			SOAPBody body = (envelope != null) ? envelope.getBody() : null;
			OMElement response = (body != null) ? body.getFirstElement() : null;
			common.saveLog(filename, common.ITI_18_RESPONSE + "_" + getQueryType(QueryUUID), response);
